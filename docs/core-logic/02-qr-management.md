...
- **Visual design customization**: `QrCode.designOptions Json?`, settable per-QR from the detail page's "Customize design" card and applied by `generateQrSvg` (`src/lib/qr/image.ts`) and the batch-download ZIP path.
  - v1 of this feature shipped with `fgColor`/`bgColor`/`logoUrl` only (solid color, centered logo overlay), and documented module/eye _shape_ customization as an out-of-scope gap on the assumption the `qrcode` library only draws square modules.
  - **That assumption was wrong, and was caught by a follow-up live audit of the competitor's actual QR-creation flow** (not just its marketing pages) — their "Customize your design" panel has six tabs (Template/Logo/Text/Color/Shape/Frame), most of which weren't in v1. Re-checked feasibility per tab: `QRCode.create(url).modules` (the `qrcode` library's own API) exposes the raw module bitmatrix, which is enough to hand-draw custom shapes in SVG without a new dependency — Shape, Frame, Text, and gradient/image Color were all buildable this way. Logo _upload_ (vs. a URL field) is the one genuine gap: it needs object storage, which this app deliberately never wired up (see Slice A note above) — swapping that in is a real infra addition, not just code, so it was deferred rather than built silently. Template (bundled presets of the above) was also deferred as low-value UI sugar until the underlying pieces existed.
  - **v2 of this feature** (SVG-only — PNG export keeps the v1 solid-color/logo behavior, a deliberate scope boundary since raster shape/frame/text compositing would need a canvas library): `moduleShape` (`square`/`rounded`/`dots`/`diamond`) and `eyeShape` (`square`/`rounded`/`circle`) are rendered by walking the raw bitmatrix and drawing each module/finder-pattern as its own SVG shape, bypassing the library's built-in `<path>` renderer entirely (`renderCustomQrBody()`). `gradient` (`{from, to, direction}`) renders as an SVG `<linearGradient>` in place of a flat `fgColor`; `bgImageUrl` tiles an image behind the modules via an SVG `<pattern>`. `frame` (`none`/`square`/`rounded`/`scan-me`) wraps the QR in a decorative border, with `scan-me` adding a captioned bar (`frameText`, default "SCAN ME") below it. `headerText`/`footerText` render above/below the whole composition. All of the above compose with the existing logo overlay and physical `sizeMm` sizing. The fast path (`QRCode.toString()`'s native SVG renderer) is kept unchanged for the common case — a plain `fgColor`/`bgColor`/`logoUrl` request never touches the custom renderer, so existing output is byte-identical to before this addition.
